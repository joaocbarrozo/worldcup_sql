...
echo  "$($PSQL "SELECT t.name AS team_name FROM (SELECT winner_id AS team_id FROM games WHERE
year = 2014 AND round = 'Eighth-Final' UNION SELECT opponent_id AS team_id FROM games WHERE year = 2014 AND round = 'Eighth-Final') AS combined JOIN teams t ON combined.team_id = t.team_id ORDER BY t.name")"
